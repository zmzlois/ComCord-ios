...
//  ComCord-mobile
//
//  Created by Zhaomian Zhao on 30/03/2024.
//

import Foundation

// Keeps time for a daily scrum meeting and keep track of the total meeting time, the time for each speaker, and the name of the current speaker
...
final class ScrumTimer: ObservableObject {
    ...
    // A closure that is executed when a new attendees begins speaking
...
extension Array<DailyScrum.Attendee> {
    var speakers: [ScrumTimer.Speaker] {
        if isEmpty {
            return [ScrumTimer.Speaker(name: "Speaker 1", isCompleted: true)]
        } else {
